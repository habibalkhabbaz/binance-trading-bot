...
const getInterval = (logger, symbolConfiguration, symbolOverrideData) => {
  ...
  // If override data is triggered by auto-trigger and TradingView override interval is configured, then use it.
  if (
...
const retrieveTradingView = async (logger, symbols, interval) => {
  ...
  try {
    const response = await axios.get('http://tradingview:8082', {
      params,
    ...
    _.forIn(tradingViewResult, (result, symbolKey) => {
      ...
      // If new recommendation is different from previous recommendation,
      if (
 ...
 * @param {*} serverLogger
 */
const runTradingView = async serverLogger => {
  const logger = serverLogger.child({ server: 'tradingview' });

  let tradingViewRunning = false;

  const tradingViewCronJob = new CronJob(
    '* * * * * *',
    async () => {
      if (tradingViewRunning) {
        logger.warn('TradingView is running, skip this tick');
        return;
      }

      tradingViewRunning = true;
      ...
      if (_.isEmpty(symbols)) {
        logger.warn('No symbols configured. Do not process get-trading-view');
        return;
      }
      ...
      tradingViewRunning = false;
    },
    null,
    false,
    config.get('tz')
  );
  tradingViewCronJob.start();
};

module.exports = { runTradingView };
